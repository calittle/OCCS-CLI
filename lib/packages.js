...
async function downloadPackage(session, pkg, outputDir, verbose, onlyActiveVersions = false, report) {
  ...
  for (const v of (onlyActiveVersions ? activeVersions(versions, 'CommunicationPackageVersionConfigRec') : versions)) {
    ...
    if (versionInfo?.DocumentJSONPathAssemblyTemplate?.Location) {
      const endpoint = `/api/CommunicationPackage/v1/CommunicationPackageVersionConfigRec/${pkgVersionUuid}/CommunicationPackageVersionConfigInfo/DocumentJSONPathAssemblyTemplate`;
      const at = await getArtifact(session, endpoint, {}, verbose, report);
      if (at) writeJSON(path.join(versionDir, 'AssemblyTemplate.json'), at);
      else if (at === null) return false;
      else {
        // Some CCS package versions expose a template Location but return an
        // empty body. Surface that tenant-data defect without preventing the
        // rest of the cache (and its mockups) from completing.
        console.warn(`⚠ Ignored empty assembly template: ${endpoint}`);
        report?.addIgnored(endpoint, { response: { status: 'empty response' } });
      }
    }
  }
  return true;
...
export async function listPackagesCommand(cmd) {
  ...
  const results = await mapWithConcurrency(packages, cmd.concurrency ?? 4, async (pkg) => {
    ...
    // A malformed CCS index row cannot be downloaded, but must not block
    // unrelated cache refreshes or downstream mockup generation.
    if (!uuid || !shortName) {
      console.warn('⚠ Ignored malformed package index row (missing UUID or short name).');
