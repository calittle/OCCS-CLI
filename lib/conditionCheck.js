...
function pathHasFilter(pathExpr) {
  return normalizePath(pathExpr).includes('[?(');
}
...
function evaluateEmptyCheck(json, fragment, pathExpr, expectEmpty) {
  ...
  if (parentRes.values.length === 0) {
    if (expectEmpty && pathHasFilter(pathExpr)) {
      return {
        passed: false,
        fragment,
        detail: `filtered parent path missing: ${parentRes.path}; Comms-compatible check treats this as not empty`,
      };
    }
...
function formatCheckDetail(detail) {
  ...
  if (/filtered parent path missing/i.test(text)) {
    return 'actual value: missing filtered parent path; add an explicit null/empty guard or emit an empty array';
  }
...
function extractSampleSignal(detail) {
  ...
  if (/filtered parent path missing/i.test(text)) {
    return 'missing filtered parent path';
  }
