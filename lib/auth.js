...
import { SESSION_PATH, loadSession, saveSession } from './session.js';
...
async function promptForMissingLoginData(opts) {
  ...
  const sessionName = String(opts.session || '').trim();
  if (sessionName) {
    try {
      const savedSession = loadSession({ sessionName, includeCredentials: true });
      ...
      for (const field of ['username', 'password', 'customer', 'region', 'tenancy']) {
        if (!explicitValues[field] && savedSession[field]) {
          credentials[field] = String(savedSession[field]);
          sources[field] = `session:${sessionName}`;
        }
      }
    } catch {
      // A new alias has no saved session yet; use explicit options or environment defaults.
    }
  }
