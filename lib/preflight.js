...
const STANDALONE_ASSET_TOP_REC_NAMES = new Map([
  ['communicationfontconfigrec', 'Font'],
  ['communicationfontversionconfigrec', 'Font'],
  ['communicationstyleconfigrec', 'Style'],
  ['communicationstyleversionconfigrec', 'Style'],
]);
const REGISTRY_RECORD_LABELS = new Map([
  ['communicationemailversionconfigcommunicationpackageconfigrelrec', 'Email version → package association'],
  ['communicationlayoutconfigcommunicationstyleconfigrelrec', 'Layout → style association'],
  ['companycommunicationdocumentconfigrelrec', 'Company → document association'],
  ['companycommunicationemailconfigrelrec', 'Company → email association'],
  ['companycommunicationpackageconfigrelrec', 'Company → package association'],
]);
...
function getConfigurationId(item) {
  ...
  const candidates = [
    ...
    item.ConfigurationInfo?.ConfigId,
    item.configurationInfo?.ConfigurationId,
    item.configurationInfo?.ConfigId,
    item.configurationInfo?.configurationId,
  ];
...
function getStatusEntries(item) {
  ...
  if (statusValue && typeof statusValue === 'object') {
    if (Array.isArray(statusValue.Items)) {
      return statusValue.Items;
    }
...
// Package detail responses expose their document links under
// CommunicationPackageVersionDocuments.  Unlike the registry response, these
// links point to a document *config* UUID (not a version UUID), which is the
// useful level for answering "which ConfigID owns the document?".
export function collectPackageDocumentDependencies(payload) {
  const dependencies = [];
  const seen = new Set();
  ...
  function visit(node) {
    ...
    const relation = node.CommunicationPackageVersionConfigCommunicationDocumentConfigRelRec;
    const info = relation?.CommunicationPackageVersionConfigCommunicationDocumentConfigRelInfo;
    const documentConfigUuid = normalizeText(info?.CommunicationDocumentConfigUuid);
    const packageVersionRecUuid = normalizeText(info?.CommunicationPackageVersionConfigUuid);

    if (documentConfigUuid) {
      const key = `${packageVersionRecUuid}::${documentConfigUuid}`;
      if (!seen.has(key)) {
        seen.add(key);
        dependencies.push({
          packageVersionRecUuid,
          documentConfigUuid,
          documentName: normalizeText(node.ShortName || node.shortName),
          ownerShortName: normalizeText(node.ConfigIdShortName || node.configIdShortName),
          ownerStatus: normalizeText(node.ConfigIdStatus || node.configIdStatus),
          relationStatusCode: getStatusCode(relation),
        });
      }
    }
...
export function collectDocumentLayoutDependencies(payload) {
  const layouts = [];
  const seen = new Set();
  const entries = payload?.CommunicationDocumentVersionLayouts;
  ...
  for (const entry of items) {
    const relation = entry?.CommunicationDocumentVersionConfigCommunicationLayoutConfigRelRec;
    const info = relation?.CommunicationDocumentVersionConfigCommunicationLayoutConfigRelInfo;
    const configUuid = normalizeText(info?.CommunicationLayoutConfigUuid);

    if (!configUuid || seen.has(configUuid)) {
      continue;
    }
    ...
    layouts.push({
      ...
      name: normalizeText(
        entry.ShortName ||
        entry.shortName ||
        entry.CommunicationLayoutConfigRec?.CommunicationLayoutConfigInfo?.ShortName ||
        entry.communicationLayoutConfigRec?.communicationLayoutConfigInfo?.shortName,
      ),
      ownerShortName: normalizeText(entry.ConfigIdShortName || entry.configIdShortName),
      ownerStatus: normalizeText(entry.ConfigIdStatus || entry.configIdStatus),
      relationStatusCode: getStatusCode(relation),
    });
  }
...
export function collectRegistryChanges(payload) {
  const changes = [];
  const seen = new Set();

  function collectRegistryEntries(node) {
    if (!node || typeof node !== 'object') {
      return;
    }

    if (Array.isArray(node)) {
      for (const entry of node) collectRegistryEntries(entry);
      return;
    }

    const topRecName = getTopRecName(node);
    const recUuid = getRecUuid(node);
    if (topRecName && recUuid) {
      const operation = normalizeText(node.Operation || node.operation) || 'Unknown';
      const key = `${topRecName}::${recUuid}::${operation}`;
      if (!seen.has(key)) {
        seen.add(key);
        changes.push({ topRecName, recUuid, operation });
      }
    }

    for (const value of Object.values(node)) {
      if (value && typeof value === 'object') collectRegistryEntries(value);
    }
  }
  ...
  function visit(node) {
    ...
    for (const [key, value] of Object.entries(node)) {
      if (key === 'ConfigurationDomainRegistryRec') {
        collectRegistryEntries(value);
      }
  ...
  return changes.sort((left, right) =>
    left.topRecName.localeCompare(right.topRecName) ||
    left.operation.localeCompare(right.operation) ||
    left.recUuid.localeCompare(right.recUuid),
  );
}

export function collectStandaloneAssetChanges(payload) {
  return collectRegistryChanges(payload)
    .map((change) => ({
      ...change,
      type: STANDALONE_ASSET_TOP_REC_NAMES.get(change.topRecName.toLowerCase()),
    }))
    .filter((change) => Boolean(change.type));
}

function registryResourceDomain(topRecName) {
  if (/^CommunicationPackage/i.test(topRecName)) return 'CommunicationPackage';
  if (/^CommunicationEmail/i.test(topRecName)) return 'CommunicationEmail';
  if (/^Communication(Document|Layout|Content|Style|Font)/i.test(topRecName)) return 'CommunicationDocument';
  return '';
}

function isDirectRegistryResource(topRecName) {
  return /(?:ConfigRec|VersionConfigRec)$/i.test(topRecName) && !/RelRec$/i.test(topRecName);
}

export function describeRegistryRecord(topRecName) {
  const normalized = normalizeText(topRecName);
  const known = REGISTRY_RECORD_LABELS.get(normalized.toLowerCase());
  if (known) return known;

  const withoutSuffix = normalized
    .replace(/VersionConfigRec$/i, ' version')
    .replace(/ConfigRec$/i, '')
    .replace(/^Communication/i, '');
  return withoutSuffix.replace(/([a-z])([A-Z])/g, '$1 $2') || normalized;
}

export function describeRegistryOperation(operation, topRecName) {
  const normalizedOperation = normalizeText(operation).toUpperCase();
  const isAssociation = /RelRec$/i.test(normalizeText(topRecName));

  if (!isAssociation) {
    return normalizedOperation === 'UPDATE_REPLACE' ? 'UPDATE' : normalizedOperation;
  }

  if (normalizedOperation === 'CREATE') return 'ASSOCIATE';
  if (normalizedOperation === 'DELETE') return 'DEASSOCIATE';
  if (normalizedOperation === 'UPDATE_REPLACE' || normalizedOperation === 'UPDATE') return 'UPDATE_ASSOCIATION';
  return normalizedOperation;
}

function collectDocumentVersionReferences(payload) {
  const entries = payload?.CommunicationDocumentMasterVersions;
  const items = Array.isArray(entries) ? entries : entries?.Items;
  ...
  return items.map((entry) => {
    const record = entry?.CommunicationDocumentVersionConfigRec || entry;
    return {
      recUuid: normalizeText(record?.CommunicationDocumentVersionConfigUuid),
      versionName: normalizeText(record?.CommunicationDocumentVersionConfigInfo?.ShortName),
    };
  }).filter((version) => version.recUuid);
}

async function resolveDocumentLayouts(session, documentMasterPayload, verbose = false) {
  const versions = collectDocumentVersionReferences(documentMasterPayload);
  const versionPayloads = await Promise.all(versions.map(async (version) => ({
    version,
    payload: await getOrNull(
      session,
      `/api/CommunicationDocument/v1/CommunicationDocumentVersionMasterConfig/${encodeURIComponent(version.recUuid)}`,
      {},
      verbose,
    ),
  })));

  return versionPayloads.flatMap(({ version, payload }) =>
    collectDocumentLayoutDependencies(payload).map((layout) => ({
      ...layout,
      documentVersion: version.versionName,
    })),
  );
}

function extractRegistryRecordName(payload, topRecName) {
  const record = payload?.[topRecName] || payload || {};
  const infoKey = topRecName.replace(/Rec$/, 'Info');
  const info = record[infoKey] || {};
  return normalizeText(info.ShortName || info.Name);
}

async function resolveRegistryChanges(session, registryChanges, verbose = false) {
  return Promise.all(registryChanges.map(async (change) => {
    const label = describeRegistryRecord(change.topRecName);
    const domainName = registryResourceDomain(change.topRecName);
    const canResolve =
      change.operation.toUpperCase() !== 'DELETE' &&
      domainName &&
      isDirectRegistryResource(change.topRecName);

    if (!canResolve) {
      return { ...change, label, name: '' };
    }

    try {
      const payload = await getOrNull(
        session,
        `/api/${domainName}/v1/${change.topRecName}/${encodeURIComponent(change.recUuid)}`,
        { depth: true },
        verbose,
      );
      return {
        ...change,
        label,
        name: payload ? extractRegistryRecordName(payload, change.topRecName) : '',
      };
    } catch (err) {
      if (verbose) {
        console.log(chalk.gray(`  ↳ could not resolve ${change.topRecName}:${change.recUuid}; retaining registry metadata`));
      }
      return { ...change, label, name: '' };
    }
  }));
...
function standaloneAssetsFromRegistryChanges(registryChanges) {
...
async function resolveDomainMaster(session, domainName, configUuid, verbose = false, cache = {}) {
  const cacheKey = `${domainName}::${configUuid}`;
  ...
  try {
    const payload = await getOrNull(
      session,
      `/api/${domainName}/v1/${domainName}MasterConfig/${encodeURIComponent(configUuid)}`,
      { depth: true },
      verbose,
    );
    ...
    const identity = extractConfigurationIdentity(payload);
    const layouts = domainName === 'CommunicationDocument'
      ? await resolveDocumentLayouts(session, payload, verbose)
      : [];
    cache[cacheKey] = {
      domainName,
      domainNameDisplay: toDisplayDomainName(domainName),
      configUuid,
      name: extractDomainName(payload, domainName),
      ownerConfigurationId: identity.configurationId,
      ownerShortName: identity.shortName,
      ownerLongName: identity.longName,
      layouts,
    ...
    handleAxiosError(err, `Failed to resolve ${domainName} ${configUuid}`);
    process.exit(1);
  }
}

async function collectDeepDependencyChains(session, configuration, attachedRows, verbose = false) {
  const packages = attachedRows.filter((row) => row.domainName === 'CommunicationPackage' && isVersionRecordRow(row));
  const masterCache = {};
  const dependencyGroups = await Promise.all(packages.map(async (row) => {
    const packagePayload = await getOrNull(
      session,
      `/api/CommunicationPackage/v1/CommunicationPackageVersionMasterConfig/${encodeURIComponent(row.recUuid)}`,
      {},
      verbose,
    );
    if (!packagePayload) {
    ...
    const packageVersion = {
      ...
      domainNameDisplay: row.domainNameDisplay,
      recUuid: row.recUuid,
      name: row.name,
      versionName: row.versionName,
      statusCode: row.statusCode,
      ownerConfigurationId: configuration.configurationId,
      ownerShortName: configuration.shortName,
      ownerLongName: configuration.longName,
    };
    return collectPackageDocumentDependencies(packagePayload).map((dependency) => ({ packageVersion, dependency }));
  }));
  const links = dependencyGroups.flat();
  const uniqueDocumentConfigUuids = [...new Set(links.map(({ dependency }) => dependency.documentConfigUuid))];
  const resolvedDocumentEntries = await Promise.all(uniqueDocumentConfigUuids.map(async (configUuid) => [
    configUuid,
    await resolveDomainMaster(session, 'CommunicationDocument', configUuid, verbose, masterCache),
  ]));
  const resolvedDocuments = new Map(resolvedDocumentEntries);

  const chains = links.map(({ packageVersion, dependency }) => {
    const resolvedDocument = resolvedDocuments.get(dependency.documentConfigUuid);
    const documentVersion = {
      domainName: 'CommunicationDocument',
      domainNameDisplay: 'Document',
      configUuid: dependency.documentConfigUuid,
      name: resolvedDocument?.name || dependency.documentName,
      versionName: '',
      ownerConfigurationId: resolvedDocument?.ownerConfigurationId || '',
      ownerShortName: resolvedDocument?.ownerShortName || dependency.ownerShortName,
      ownerLongName: resolvedDocument?.ownerLongName || '',
      ownerStatus: dependency.ownerStatus,
      layouts: resolvedDocument?.layouts || [],
    };
    const ownerIdentity = {
    ...
    return {
      relationStatusCode: dependency.relationStatusCode,
      packageVersion,
      documentVersion,
      externalOwner,
    };
  ...
  chains.sort((left, right) =>
    ...
    (left.documentVersion.configUuid || '').localeCompare(right.documentVersion.configUuid || '', undefined, { sensitivity: 'base' }),
  );
  return chains;
}
...
async function analyzeConfiguration(session, configuration, verbose = false) {
  ...
  if (configuration.listAttached || configuration.deep) {
  ...
  if (configuration.deep) {
    deepChains = await collectDeepDependencyChains(session, configuration, attachedRows, verbose);
    registryChanges = await resolveRegistryChanges(
      session,
      collectRegistryChanges(detailPayload),
      verbose,
    );
    standaloneAssets = standaloneAssetsFromRegistryChanges(registryChanges);
  }
...
function renderDeepSummary(results) {
  for (const result of results) {
    const deepChains = Array.isArray(result.deepChains) ? result.deepChains : [];
    console.log(`\nDeep package/document dependencies for ${chalk.cyan(result.configurationId)}${result.shortName ? ` (${result.shortName})` : ''}:`);

    if (!deepChains.length) {
      console.log(chalk.yellow('  No package-to-document associations were found.'));
      continue;
    }
    ...
    for (const chain of deepChains) {
      const packageName = chain.packageVersion.name || chain.packageVersion.recUuid;
      const packageVersion = chain.packageVersion.versionName || '--';
      const documentName = chain.documentVersion.name || chain.documentVersion.configUuid;
      const ownerLabel = formatOwnerLabel(chain.documentVersion);
      const externalLabel = chain.externalOwner ? chalk.yellow('cross-config') : chalk.green('same-config');
      console.log(`  Package ${packageName} | ${packageVersion}`);
      console.log(`    → Document ${documentName} | owner ${ownerLabel} | ${externalLabel} | relation ${chain.relationStatusCode || 'Unknown'}`);
      for (const layout of chain.documentVersion.layouts || []) {
        const layoutIdentity = { shortName: layout.ownerShortName };
        const layoutExternal = layout.ownerShortName && !isSameConfigurationIdentity(result, layoutIdentity);
        const layoutOwner = layout.ownerShortName || 'Unknown';
        const layoutName = layout.name || layout.configUuid;
        const layoutVersion = layout.documentVersion ? ` | document version ${layout.documentVersion}` : '';
        console.log(`      ↳ Layout ${layoutName}${layoutVersion} | owner ${layoutOwner} | ${layoutExternal ? chalk.yellow('cross-config') : chalk.green('same-config')} | relation ${layout.relationStatusCode || 'Unknown'}`);
      }
    }
  }
}

function renderStandaloneAssetSummary(results) {
  for (const result of results) {
    const assets = Array.isArray(result.standaloneAssets) ? result.standaloneAssets : [];
    console.log(`\nStandalone Font and Style changes for ${chalk.cyan(result.configurationId)}${result.shortName ? ` (${result.shortName})` : ''}:`);
    ...
    for (const asset of assets) {
      console.log(`  ${asset.type}: ${asset.name || '(name unavailable)'} | ${asset.operation} | ${asset.topRecName} | ${asset.recUuid}`);
    }
  }
}

function renderRegistryChangeSummary(results) {
  for (const result of results) {
    const changes = Array.isArray(result.registryChanges) ? result.registryChanges : [];
    console.log(`\nConfiguration registry changes for ${chalk.cyan(result.configurationId)}${result.shortName ? ` (${result.shortName})` : ''}:`);
    ...
    for (const change of changes) {
      ...
      const operation = describeRegistryOperation(change.operation, change.topRecName);
      console.log(`  ${operation}: ${change.label || describeRegistryRecord(change.topRecName)}${name} | ${change.recUuid}`);
    }
  }
}
...
export async function preflightCommand(cmd) {
  ...
  if (cmd.deep) {
    console.log(chalk.cyan('ℹ Deep scan enabled: resolving registry changes plus package, document, and layout dependencies. This can take several minutes for configs with many references.\n'));
  }
  ...
  for (const configuration of configurationsToScan) {
    ...
    writeJSON(path.join(artifactDir, `${safePathSegment(configuration.configurationId)}.json`), {
      ...
      deepChains: result.deepChains,
      standaloneAssets: result.standaloneAssets,
  ...
  if (cmd.deep) {
    renderDeepSummary(results);
    renderRegistryChangeSummary(results);
    renderStandaloneAssetSummary(results);
  }
