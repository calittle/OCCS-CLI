...
export function parseContentBlobs(baseDir, options = {}) {
  // TD generation downloads a deliberately scoped content catalogue directly
  // into its destination.  The full OCCS export still uses <base>/contents.
  const contentRoot = options.contentRoot || path.join(baseDir, 'contents');
