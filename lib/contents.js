...
import { get, mutateJson, paginate, postMultipart, putMultipart } from './api.js';
  ...
  return {
    ...
    CommunicationContentVersionConfigRec: {
      CommunicationContentVersionConfigInfo: {
        ...
        Desc: String(versionDescription ?? ''),
...
export function buildContentFileMultipart(filePath) {
...
/**
 * OCCS accepts only the relationship portion of a style attachment on a
 * version-master update.  The GET response expands each relationship with
 * status and the complete Style record; replaying those response-only fields
 * causes the update endpoint to reject the otherwise valid request.
 */
export function buildVersionUpdateStyles(sourceStyles, versionUuid, configId) {
  return collectionItems(sourceStyles)
    .map((entry) => entry?.CommunicationStyleConfigCommunicationContentVersionConfigRelRec || entry || {})
    .map((record) => {
      const info = record.CommunicationStyleConfigCommunicationContentVersionConfigRelInfo || {};
      if (!info.CommunicationStyleConfigUuid) return null;
      return {
        CommunicationStyleConfigCommunicationContentVersionConfigRelRec: {
          CommunicationStyleConfigCommunicationContentVersionConfigRelInfo: {
            ...info,
            CommunicationContentVersionConfigUuid: versionUuid,
            ConfigId: String(configId),
          },
          ...(record.CommunicationStyleConfigCommunicationContentVersionConfigRelUuid
            ? { CommunicationStyleConfigCommunicationContentVersionConfigRelUuid: record.CommunicationStyleConfigCommunicationContentVersionConfigRelUuid }
            : {}),
        },
      };
    })
...
export async function contentListCommand(cmd) {
  ...
  return runContentCommand(opts, async () => {
    ...
    const requestedLimit = String(opts.limit || '').trim();
    const limit = requestedLimit
      ? Math.min(Math.max(Number.parseInt(requestedLimit, 10) || 50, 1), 100)
      : null;
    ...
    const pages = await Promise.all(searchFields.map(async (field) => {
      ...
      if (limit) {
        return get(session, `${CONTENT_API}/CommunicationContentConfigRec`, { ...params, offset: 0, limit }, opts.verbose, { timeout: opts.timeout });
      }
      return { Items: await paginate(session, `${CONTENT_API}/CommunicationContentConfigRec`, params, 100, opts.verbose, { timeout: opts.timeout }) };
    }));
    ...
    const matchingContents = filterContentBrowserItems(scoped, filter)
      .filter((item) => !contentType || item.contentType.toLowerCase() === contentType.toLowerCase());
    const contents = limit ? matchingContents.slice(0, limit) : matchingContents;
    const truncated = Boolean(limit) && (pages.some((page) => page?.HasMore || Number(page?.TotalResults || 0) > collectionItems(page).length) || matchingContents.length > contents.length);
    writeResult(opts, {
      ok: true,
...
export async function contentSaveCommand(contentNameOrUuid, version, cmd) {
  ...
  return runContentCommand(opts, async () => {
    ...
    const contentMetadataChanged = (
      (opts.name !== undefined && opts.name !== existingContentInfo.Name)
      || (opts.shortName !== undefined && opts.shortName !== existingContentInfo.ShortName)
      || (opts.desc !== undefined && opts.desc !== (existingContentInfo.Desc ?? ''))
    );
    const versionMetadataChanged = (
      (opts.newVersion !== undefined && opts.newVersion !== existingVersionInfo.ShortName)
      || (opts.versionDesc !== undefined && opts.versionDesc !== (existingVersionInfo.Desc ?? ''))
    );
        ...
        return {
          CommunicationContentVersionConfigRec: {
            CommunicationContentVersionConfigInfo: withConfig({
              ...info,
              CommunicationContentVersionConfigData: collectionItems(info.CommunicationContentVersionConfigData).map(withConfig),
            }),
    ...
    const compactStyles = buildVersionUpdateStyles(
      versionMaster.CommunicationContentVersionStyles,
      selectedVersionUuid,
      configId,
    );
    if (contentMetadataChanged) {
    ...
    const versionUpdate = {
      ...
      CommunicationContentVersionStyles: compactStyles,
    };
    ...
    if (opts.html) await uploadContentBlob(session, selectedVersionUuid, readHtmlFile(opts.html), configId, opts, { replace: true });
...
async function uploadContentBlob(session, targetVersionUuid, html, configId, opts, { replace = false } = {}) {
  const { boundary, body } = buildContentBlobMultipart(html);
  const upload = replace ? putMultipart : postMultipart;
  return upload(
    session,
...
async function uploadContentFile(session, targetVersionUuid, filePath, configId, opts) {
  const { boundary, body } = buildContentFileMultipart(filePath);
  const response = await postMultipart(
    session,
    `${CONTENT_API}/CommunicationContentVersionConfigRec/${targetVersionUuid}/CommunicationContentVersionConfigInfo/CommunicationContentVersionConfigData/1/ContentData?topRecResource=CommunicationContentVersionConfigRec&responseAsJson=true`,
    body,
    { headers: { 'Content-Type': `multipart/form-data; boundary=${boundary}`, 'Content-Length': String(body.length), transactionconfigid: String(configId) }, verbose: opts.verbose, timeout: opts.timeout },
  );
  // Image upload returns server-derived dimensions/format. Persist precisely
  // those values rather than trying to interpret image files in the CLI.
  if (response?.CommunicationContentVersionConfigImageData) {
    await mutateJson(session, 'PUT', `${CONTENT_API}/CommunicationContentVersionConfigRec/${targetVersionUuid}/CommunicationContentVersionConfigInfo/CommunicationContentVersionConfigData/1/CommunicationContentVersionConfigImageData`, {
      ...response.CommunicationContentVersionConfigImageData,
      ConfigId: String(configId),
...
export async function contentCreateCommand(shortName, cmd) {
  ...
  return runContentCommand(opts, async () => {
    ...
    const contentType = String(opts.type || 'Text');
    const isImage = contentType.toLowerCase() === 'image';
    if (isImage && !opts.file) throw new Error('--file is required when --type Image.');
    if (!isImage && !opts.html) throw new Error('--html is required unless --type Image.');
    if (opts.file && !fs.existsSync(opts.file)) throw new Error(`Content file not found: ${opts.file}`);
    const html = opts.html ? readHtmlFile(opts.html) : null;
    const session = loadSession(sessionSelector(opts));
    const config = await resolveOpenConfigId(session, configInput, opts);
    const configId = config.resolved;
    if (opts.dryRun) {
      writeResult(opts, {
        ok: true, dryRun: true, operation: 'create-content', configId: config, payload,
        ...(html !== null ? { htmlBytes: Buffer.byteLength(html) } : { file: opts.file, fileBytes: fs.statSync(opts.file).size }),
      }, [
        ...
        ...(html !== null ? [`HTML: ${opts.html} (${Buffer.byteLength(html)} bytes)`] : [`File: ${opts.file} (${fs.statSync(opts.file).size} bytes)`]),
    ...
    const blob = isImage
      ? await uploadContentFile(session, createdVersionUuid, opts.file, configId, opts)
      : await uploadContentBlob(session, createdVersionUuid, html, configId, opts);
    writeResult(opts, {
      ok: true, operation: 'create-content', configId: config, contentUuid: contentUuid(created), versionUuid: createdVersionUuid,
      ...(html !== null ? { htmlBytes: Buffer.byteLength(html) } : { file: opts.file, fileBytes: fs.statSync(opts.file).size }), blob,
    }, [
      ...
      html !== null ? `HTML uploaded: ${Buffer.byteLength(html)} bytes.` : `File uploaded: ${path.basename(opts.file)}.`,
    ]);
  });
}

/**
 * Clone a content item into a new content master. OCCS content references in
 * the HTML are deliberately preserved verbatim; this copies one item, not a
 * recursive dependency graph.
 */
export async function contentDuplicateCommand(sourceNameOrUuid, targetShortName, cmd) {
  const opts = optsWithGlobals(cmd);
  return runContentCommand(opts, async () => {
    const configInput = requireValue(opts.configId, 'Config ID');
    const session = loadSession(sessionSelector(opts));
    const config = await resolveOpenConfigId(session, configInput, opts);
    const configId = String(config.resolved);
    const source = await resolveContent(session, sourceNameOrUuid, opts);
    const contentMaster = await get(session, `${CONTENT_API}/CommunicationContentMasterConfig/${source.resolved}`, { depth: true, limit: 30 }, opts.verbose, { timeout: opts.timeout, throwOnError: true });
    const sourceVersion = resolveSourceContentVersion(contentMaster, requireValue(opts.fromVersion, 'Source version'));
    const sourceVersionUuid = sourceVersion.CommunicationContentVersionConfigUuid;
    if (!sourceVersionUuid) throw new Error(`Content version ${opts.fromVersion} did not include a UUID.`);
    ...
    const sourceContent = unwrapContentRecord(contentMaster.CommunicationContentConfigRec || contentMaster);
    const sourceContentInfo = sourceContent.CommunicationContentConfigInfo || {};
    const sourceVersionInfo = sourceVersionMaster?.CommunicationContentVersionConfigRec?.CommunicationContentVersionConfigInfo || {};
    const sourceData = collectionItems(sourceVersionInfo.CommunicationContentVersionConfigData)[0] || {};
    const location = sourceData?.ContentData?.Location;
    if (!location) throw new Error(`Content version ${opts.fromVersion} has no HTML data.`);
    const copiedStyles = buildCopiedVersionStyles(sourceVersionMaster.CommunicationContentVersionStyles, configId);
    const payload = buildCreateContentPayload({
      shortName: targetShortName,
      name: opts.name || targetShortName,
      description: opts.desc ?? sourceContentInfo.Desc ?? '',
      contentType: sourceContentInfo.ContentType || 'Text',
      version: opts.version || sourceVersionInfo.ShortName || '1.0',
      versionDescription: opts.versionDesc ?? sourceVersionInfo.Desc ?? '',
      effectiveDate: opts.effectiveDate || todayUtcDate(),
      styleClasses: sourceStyleClasses(sourceVersionMaster),
    });
    payload.CommunicationContentVersionStyles = copiedStyles;
    if (opts.dryRun) {
      writeResult(opts, {
        ok: true, dryRun: true, operation: 'duplicate-content', configId: config, source,
        sourceVersion: opts.fromVersion, payload, copiedStyles: copiedStyles.length,
      }, ['Dry run: no OCCS changes made.']);
      return;
    }
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'occs-content-duplicate-'));
    try {
      const htmlPath = path.join(tempDir, 'content.html');
      if (!await downloadBlob(session, location, htmlPath, opts.verbose)) throw new Error(`Could not download HTML for content version ${opts.fromVersion}.`);
      const html = fs.readFileSync(htmlPath, 'utf8');
      const created = await mutateJson(session, 'POST', `${CONTENT_API}/CommunicationContentVersionMasterConfig`, payload, {
        headers: { transactionconfigid: configId }, verbose: opts.verbose, timeout: opts.timeout,
      });
      const targetVersionUuid = versionUuid(created);
      const targetContentUuid = contentUuid(created);
      if (!targetVersionUuid || !targetContentUuid) throw new Error('OCCS create response did not include content and version UUIDs.');
      const update = buildVersionMasterUpdatePayload({
        createdVersion: created,
        contentRecord: created.CommunicationContentConfigRec,
        configId,
        versionStyles: buildVersionUpdateStyles(sourceVersionMaster.CommunicationContentVersionStyles, targetVersionUuid, configId),
      });
      await mutateJson(session, 'PUT', `${CONTENT_API}/CommunicationContentVersionMasterConfig/${targetVersionUuid}`, update, {
        headers: { transactionconfigid: configId }, verbose: opts.verbose, timeout: opts.timeout,
      });
      await uploadContentBlob(session, targetVersionUuid, html, configId, opts);
      const verified = await get(session, `${CONTENT_API}/CommunicationContentVersionMasterConfig/${targetVersionUuid}`, { depth: true, limit: 40 }, opts.verbose, { timeout: opts.timeout, throwOnError: true });
      const verifiedStyles = contentVersionStyles(verified).length;
      writeResult(opts, {
        ok: true, operation: 'duplicate-content', configId: config, source, sourceVersion: opts.fromVersion,
        contentUuid: targetContentUuid, versionUuid: targetVersionUuid, htmlBytes: Buffer.byteLength(html),
        copiedStyles: verifiedStyles,
      }, [
        `Duplicated content ${source.shortName || source.resolved} as ${targetShortName}.`,
        `Copied ${verifiedStyles} style relationship(s); uploaded ${Buffer.byteLength(html)} HTML bytes.`,
      ]);
    } finally {
      fs.rmSync(tempDir, { recursive: true, force: true });
    }
...
export async function listContentsCommand(cmd) {
  ...
  const results = await mapWithConcurrency(contents, cmd.concurrency ?? 4, async (item) => {
    ...
    // Ignore CCS index rows that cannot identify content to download.
    if (!uuid || !shortName) {
      console.warn('⚠ Ignored malformed content index row (missing UUID or short name).');
