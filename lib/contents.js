...
export async function contentListCommand(cmd) {
  ...
  return runContentCommand(opts, async () => {
    ...
    const scoped = records.filter((item) => {
      ...
      return !id || id === String(config.resolved);
    });
    const contents = filterContentBrowserItems(scoped, opts.filter);
    writeResult(opts, {
      ok: true,
      ...
      `Found ${contents.length} content item${contents.length === 1 ? '' : 's'}.`,
      `ConfigId: ${configLabel(config)} (${config.resolved})`,
    ]);
  });
}
...
export async function contentInspectCommand(contentNameOrUuid, cmd) {
  ...
  return runContentCommand(opts, async () => {
    ...
    writeResult(opts, { ok: true, operation: 'inspect-content', content: contentBrowserItem(contentRecord), versions, ...(selectedVersion ? { selectedVersion, html } : {}) }, [
