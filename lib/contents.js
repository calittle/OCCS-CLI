...
export async function contentCreateCommand(shortName, cmd) {
  ...
  if (opts.dryRun) {
    writeResult({ ok: true, dryRun: true, operation: 'create-content', configId: config, payload, htmlBytes: Buffer.byteLength(html) });
    return;
  }
...
export async function contentVersionCommand(contentUuidArg, version, cmd) {
  ...
  if (opts.dryRun) {
    writeResult({ ok: true, dryRun: true, operation: 'create-content-version', configId: config, contentUuid: contentUuidValue, payload, htmlBytes: Buffer.byteLength(html) });
    return;
  }
