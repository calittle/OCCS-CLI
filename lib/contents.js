  ...
  return {
    ...
    CommunicationContentVersionConfigRec: {
      CommunicationContentVersionConfigInfo: {
        ...
        Desc: '',
        Language: requireValue(language, 'Language'),
...
export async function contentCreateCommand(shortName, cmd) {
  ...
  const configId = requireValue(opts.configId, 'Config ID');
  ...
  if (opts.dryRun) {
    writeResult({ ok: true, dryRun: true, operation: 'create-content', configId, payload, htmlBytes: Buffer.byteLength(html) });
    return;
  }
  ...
  writeResult({ ok: true, operation: 'create-content', configId, contentUuid: contentUuid(created), versionUuid: createdVersionUuid, blob });
}
...
export async function contentVersionCommand(contentUuidArg, version, cmd) {
  ...
  if (opts.dryRun) {
    writeResult({ ok: true, dryRun: true, operation: 'create-content-version', configId, contentUuid: contentUuidValue, payload, htmlBytes: Buffer.byteLength(html) });
    return;
  }
  const session = loadSession(sessionSelector(opts));
  ...
  const blob = await uploadContentBlob(session, createdVersionUuid, html, configId, opts);
  writeResult({ ok: true, operation: 'create-content-version', configId, contentUuid: contentUuidValue, versionUuid: createdVersionUuid, blob });
}
