...
export async function listDocumentsCommand(cmd) {
  ...
  for (const doc of documents) await downloadDocument(session, doc, outputDir, cmd.verbose);

  console.log(`✅ Saved ${documents.length} documents to ${outputDir}`);
}

async function downloadDocument(session, doc, outputDir, verbose, onlyActiveVersions = false) {
  ...
  const master = await get(session, `/api/CommunicationDocument/v1/CommunicationDocumentMasterConfig/${docUuid}`, { depth: true }, verbose);
  ...
  for (const v of (onlyActiveVersions ? activeVersions(versions, 'CommunicationDocumentVersionConfigRec') : versions)) {
    ...
    const versionDetails = await get(session, `/api/CommunicationDocument/v1/CommunicationDocumentVersionMasterConfig/${versionUuid}`, { depth: true }, verbose);
...
export async function getDocumentCommand(shortName, cmd) {
  ...
  await downloadDocument(session, doc, outputDir, cmd.verbose, true);
