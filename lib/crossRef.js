...
export async function crossrefCommand(cmd) {
  ...
  function getContentRefsUsedByContent(contentRefs, contentId) {
    return contentRefs.filter((ref) => ref.contentId === contentId);
  }
        ...
        const traverseLayout = (layout, parentLayoutId = null, depth = 0, pathParts = []) => {
          ...
          if (fs.existsSync(layoutFile)){
            ...
            for (const layoutContent of layoutContents) {   
                ...
                const contentReferenceUsages = getContentRefsUsedByContent(contentRefs, contentName);
