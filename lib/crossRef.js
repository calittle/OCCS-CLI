...
import { writeCSV, safePathSegment } from './utils.js';
...
export async function crossrefCommand(cmd) {
  ...
  const readJson = (file) => {
    if (!jsonCache.has(file)) jsonCache.set(file, JSON.parse(fs.readFileSync(file)));
  ...
  const fieldUsageByContent = new Map();
  const contentRefsByContent = new Map();
  ...
  // ---- Parse Field and Content Usage from Content Blobs ----
  const { usageMap, contentRefs } = parseContentBlobs(baseDir);
  for (const [fieldId, usages] of Object.entries(usageMap)) {
    for (const usage of usages) addToIndex(fieldUsageByContent, usage.contentId, { fieldId, type: usage.type });
  }
  for (const ref of contentRefs) addToIndex(contentRefsByContent, ref.contentId, ref);
  ...
  if (fs.existsSync(docsDir)) {
    for (const doc of fs.readdirSync(docsDir)) {
      ...
      const masterData = readJson(masterFile);
      ...
      for (const versionFile of fs.readdirSync(versionsDir)) {
        ...
        const traverseLayout = (layout, parentLayoutId = null, depth = 0, pathParts = []) => {
          ...
          if (fs.existsSync(layoutFile)){
            const layoutData = readJson(layoutFile);
            ...
            for (const layoutContent of layoutContents) {   
                ...
                if (fs.existsSync(contentFile)){                            
                    const contentData = readJson(contentFile);
                ...
                // CONTENT -> FIELDS
                const usages = fieldUsageByContent.get(contentName) || [];
                ...
                // CONTENT -> CONTENT references embedded in content blobs
                const contentReferenceUsages = contentRefsByContent.get(contentName) || [];
                for (const ref of contentReferenceUsages) {
                    addRow(
                      docName,
                      'Content',
                      contentName,
