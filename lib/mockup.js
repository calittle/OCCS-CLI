...
function gridCss(layouts) {
  ...
  const isFirstAreaRow = (rows, rowIndex, area) => rows.findIndex(row => row.includes(area)) === rowIndex;
  const minimumHeight = node => {
    ...
    if (String(node.type || '').toLowerCase() === 'grid' && rows.length) {
      return (isCompactBand(node) ? 34 : 42) + rows.reduce((height, row, rowIndex) => {
        const baseMinimum = isCompactBand(node) ? compactRegionHeight : minRegionHeight;
        const rowMinimum = Math.max(baseMinimum, ...row.map(area => isFirstAreaRow(rows, rowIndex, area) ? childMinimum(node, children.find(child => child.area === area) || { children: [] }) : baseMinimum));
  ...
  const inferredRows = node => {
    ...
    return rows.map((row, rowIndex) => {
      const baseMinimum = isCompactBand(node) ? compactRegionHeight : minRegionHeight;
      const rowMinimum = Math.max(baseMinimum, ...row.map(area => isFirstAreaRow(rows, rowIndex, area) ? childMinimum(node, node.children.find(child => child.area === area) || { children: [] }) : baseMinimum));
      const finalChild = node.children.find(child => child.area === row[0]);
...
const rootMinimum=node=>{
  ...
  if(String(node.type||'').toLowerCase().includes('grid')&&rows.length){
    ...
    const isFirstAreaRow=(rowIndex,area)=>rows.findIndex(row=>row.includes(area))===rowIndex;
    return labelBand+rows.reduce(
      (height,row,rowIndex)=>height+Math.max(
        childMinimum,
        ...row.map(area=>isFirstAreaRow(rowIndex,area)?(compactBand(node)?compactMinimum:rootMinimum(children.find(child=>child.area===area)||{children:[]})):childMinimum),
      ),
      0,
