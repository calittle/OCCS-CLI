...
function walkLayout(uuid, layouts, contents, stylesByUuid, seen = new Set()) {
  ...
  const children = (data.CommunicationLayoutLayouts || []).map(item => {
    ...
    return { ...walkLayout(relation.RelCommunicationLayoutConfigUuid, layouts, contents, stylesByUuid, new Set(seen)), always: relation.LayoutAlwaysTriggerInd === true, area: relation.StyleAreaName || '', order: Number(relation.LayoutRelIndex || Number.MAX_SAFE_INTEGER) };
...
export async function mockupCommand(documentName, cmd) {
  ...
  const rootLayouts = (document.CommunicationDocumentVersionLayouts || []).map(item => {
    ...
    return {
      ...walkLayout(relation.CommunicationLayoutConfigUuid, layouts, contents, stylesByUuid),
