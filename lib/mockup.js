...
function walkLayout(uuid, layouts, contents, stylesByUuid, assemblyBindings = {}, seen = new Set()) {
  ...
  for (const child of children) {
    ...
    if (!child.area) {
      if (child.always === false) child.visualSuppressed = true;
      continue;
    }
    ...
    if (existing.always === false && child.always !== false) {
...
function gridCss(layouts) {
  ...
  const minimumHeight = node => {
    ...
    if (String(node.type || '').toLowerCase() === 'grid' && rows.length) {
      return (isCompactBand(node) ? 34 : 42) + rows.reduce((height, row) => {
        const rowMinimum = Math.max(isCompactBand(node) ? compactRegionHeight : minRegionHeight, ...row.map(area => childMinimum(node, children.find(child => child.area === area) || { children: [] })));
  ...
  const inferredRows = node => {
    ...
    return rows.map(row => {
      const rowMinimum = Math.max(isCompactBand(node) ? compactRegionHeight : minRegionHeight, ...row.map(area => childMinimum(node, node.children.find(child => child.area === area) || { children: [] })));
      return 'minmax(' + rowMinimum + 'px,' + Math.max(...row.map(weightForArea)) + 'fr)';
    }).join(' ');
  };
  const visit = (node, id) => {
...
const page=document.querySelector('#page'),title=page.querySelector('.page-title'),map=document.querySelector('#document-map'),minimum=56,compactMinimum=45;page.dataset.node='document';page.classList.add('document-node');title.className='node-title';title.textContent=m.document.name;page.insertAdjacentHTML('beforeend','<span class="node-meta">Document · A4+ portrait</span>');const compactBand=node=>/(?:^|[\\s:_-])(header|footer)(?:[\\s:_-]|$)/i.test(String(node.name||''));const rootMinimum=node=>{const children=(node.children||[]).filter(child=>!child.visualSuppressed),rows=[...String(node.grid?.areas||'').matchAll(/'([^']+)'/g)].map(match=>match[1].trim().split(/\\s+/));if(!children.length)return minimum;if(String(node.type||'').toLowerCase().includes('grid')&&rows.length){const childMinimum=compactBand(node)?compactMinimum:minimum;const labelBand=compactBand(node)?34:42;return labelBand+rows.reduce((height,row)=>height+Math.max(childMinimum,...row.map(area=>compactBand(node)?compactMinimum:rootMinimum(children.find(child=>child.area===area)||{children:[]}))),0)+Math.max(0,rows.length-1)*6}return minimum+children.reduce((height,child)=>height+rootMinimum(child),0)+Math.max(0,children.length-1)*6};/* Bill and statement pages use a deliberately soft 1:2:1 header/detail/footer allocation. Header and footer cells receive a compact two-line minimum, while the cached grid styles continue to control the geometry within each region. The root minimum prevents a weighted row from collapsing over its own children. Footer rows use their content height instead of absorbing spare page height. */map.style.gridTemplateRows=m.layouts.map(node=>{const name=String(node.name).toLowerCase(),weight=name.includes('footer') ? 1 : name.includes('header') ? (name.includes('address') ? .6 : .2) : name.includes('charges') ? .2 : (name.includes('detail')||name.includes('body')||name.includes('overall_grid')) ? 2 : 1;return 'minmax('+rootMinimum(node)+'px,'+(name.includes('footer')?'auto':weight+'fr')+')'}).join(' ');
