...
function walkLayout(uuid, layouts, contents, chartsByUuid, stylesByUuid, assemblyBindings = {}, layoutBindings = {}, seen = new Set()) {
  ...
  const relations = (data.CommunicationLayoutContents || []).map(item => {
    ...
    return {
      ...contentDetails(name, contents, chartsByUuid),
  ...
  const children = (data.CommunicationLayoutLayouts || []).map(item => {
    ...
    return { ...walkLayout(layoutId, layouts, contents, chartsByUuid, stylesByUuid, assemblyBindings, layoutBindings, new Set(seen)), always: inclusion === 'never' ? 'never' : always, inclusion, condition: binding?.condition || '', area: relation.StyleAreaName || '', order: Number(relation.LayoutRelIndex || Number.MAX_SAFE_INTEGER) };
  }).sort((a, b) => a.order - b.order);
...
function packageContext(cacheDir, packageName, documentName) {
  ...
  for (const layout of doc?.Layouts || []) {
    layoutBindings[layout.$$Id] = { condition: layout.xCondition || layout.Condition || '' };
...
function gridCss(layouts) {
  ...
  // A child can span several *rows* (such as a sidebar beside a stack of
  // details). Its height must be shared across those rows. Assigning its whole
  // minimum to the first row creates a large, artificial gap below that row.
  const areaRowSpan = (rows, area) => Math.max(1, rows.reduce((count, row) => count + (row.includes(area) ? 1 : 0), 0));
  const rowContribution = (parent, rows, area) => {
    const child = (parent.children || []).find(item => item.area === area && !item.visualSuppressed);
    return child ? Math.ceil(childMinimum(parent, child) / areaRowSpan(rows, area)) : 0;
  };
  const minimumHeight = node => {
    ...
    if (String(node.type || '').toLowerCase() === 'grid' && rows.length) {
      return (isCompactBand(node) ? 34 : 42) + rows.reduce((height, row) => {
        const baseMinimum = isCompactBand(node) ? compactRegionHeight : minRegionHeight;
        const rowMinimum = Math.max(baseMinimum, ...row.map(area => rowContribution(node, rows, area) || baseMinimum));
        return height + rowMinimum;
      }, 0) + Math.max(0, rows.length - 1) * 6 + (isCompactBand(node) ? 10 : 6);
    }
  ...
  const inferredRows = node => {
    ...
    const weightForArea = area => {
      const child = node.children.find(item => item.area === area);
      // A grid that itself stacks five notices needs more room than the oval
      // and QR siblings beside it. This is an inspector readability rule, not
      // a substitute for the source document's physical layout.
      return Math.max(1, (child?.children?.length || 0) + (child?.children?.length ? 1 : 0));
    };
    return rows.map((row, rowIndex) => {
      const baseMinimum = isCompactBand(node) ? compactRegionHeight : minRegionHeight;
      const rowMinimum = Math.max(baseMinimum, ...row.map(area => rowContribution(node, rows, area) || baseMinimum));
      const finalChild = node.children.find(child => child.area === row[0] && !child.visualSuppressed);
      const absorbsSidebarRemainder = rows.every(sourceRow => sourceRow.length === 1) && rowIndex === rows.length - 1 && /(?:qr|qrcode)/i.test(String(finalChild?.name || ''));
      return 'minmax(' + rowMinimum + 'px,' + (absorbsSidebarRemainder ? '1fr' : 'max-content') + ')';
    }).join(' ');
  };
  const visit = (node, id) => {
...
const rootMinimum=node=>{
  const children=(node.children||[]).filter(child=>!child.visualSuppressed);
  const rows=[...String(node.grid?.areas||'').matchAll(/'([^']+)'/g)].map(match=>match[1].trim().split(/\\s+/));
  if(!children.length)return minimum;
  if(String(node.type||'').toLowerCase().includes('grid')&&rows.length){
    const childMinimum=compactBand(node)?compactMinimum:minimum;
    const labelBand=compactBand(node)?34:42;
    const rowSpan=area=>Math.max(1,rows.reduce((count,row)=>count+(row.includes(area)?1:0),0));
    const rowContribution=area=>{
      const child=children.find(item=>item.area===area);
      return child?Math.ceil((compactBand(node)?compactMinimum:rootMinimum(child))/rowSpan(area)):0;
    };
    return labelBand+rows.reduce(
      (height,row)=>height+Math.max(childMinimum,...row.map(area=>rowContribution(area)||childMinimum)),
      0,
...
export async function mockupCommand(documentName, cmd) {
  ...
  const rootLayouts = (document.CommunicationDocumentVersionLayouts || []).map(item => {
    ...
    return {
      ...walkLayout(relation.CommunicationLayoutConfigUuid, layouts, contents, chartsByUuid, stylesByUuid, packageInfo?.bindings, packageInfo?.layoutBindings),
  ...
  const addContent = name => {
    ...
    const detail = contentDetails(name, contents, chartsByUuid);
