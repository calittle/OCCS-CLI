...
function contentDetails(name, index) {
  ...
  return {
    name, version, effectiveDate: rec.effectiveDate || '', description: rec.info.Desc || (conditionalDetails.length ? `Conditional container for ${conditionalDetails.length} alternate content block${conditionalDetails.length === 1 ? '' : 's'}.` : ''),
...
function walkLayout(uuid, layouts, contents, stylesByUuid, assemblyBindings = {}, layoutBindings = {}, seen = new Set()) {
  ...
  const children = (data.CommunicationLayoutLayouts || []).map(item => {
    ...
    return { ...walkLayout(layoutId, layouts, contents, stylesByUuid, assemblyBindings, layoutBindings, new Set(seen)), always: inclusion === 'never' ? 'never' : always, inclusion, condition: binding?.condition || '', area: relation.StyleAreaName || '', order: Number(relation.LayoutRelIndex || Number.MAX_SAFE_INTEGER) };
  }).sort((a, b) => a.order - b.order);
...
export async function mockupCommand(documentName, cmd) {
  ...
  const rootLayouts = (document.CommunicationDocumentVersionLayouts || []).map(item => {
    ...
    return {
      ...walkLayout(relation.CommunicationLayoutConfigUuid, layouts, contents, stylesByUuid, packageInfo?.bindings, packageInfo?.layoutBindings),
