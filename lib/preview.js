...
function applyEmailRecipientOverride(inputJson, recipients) {
  ...
  const eBill = inputJson?.billPrint?.billDetails?.eBill;
  if (!eBill || typeof eBill !== 'object' || !Array.isArray(eBill.recipientEmails)) {
    throw new Error(
      'Cannot override recipients: input JSON must contain billPrint.billDetails.eBill.recipientEmails.',
    );
  }
  ...
  return {
    ...
    billPrint: {
      ...inputJson.billPrint,
      billDetails: {
        ...inputJson.billPrint.billDetails,
        eBill: {
          ...eBill,
...
export async function convertXmlCommand(opts) {
  ...
  setRuntimeCompletionContext(getConvertXmlCompletionContext(opts, session));
...
export async function previewCommand(opts) {
  ...
  if (includesEmail) {
    if (!opts.sendEmail) {
      console.error(chalk.red('❌ EMAIL sends a real email. Re-run with --send-email to acknowledge this action.'));
      exitWithErrorBell();
    }
