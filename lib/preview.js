...
async function convertXmlRecord({
  ...
  try {
    return injectDebugValue(convertedJson, debugSpec);
  } catch (injectErr) {
...
export async function convertXmlCommand(opts) {
  ...
  if (!session.username || !session.password) {
    console.error(chalk.red('❌ XML conversion requires OCCS_USERNAME and OCCS_PASSWORD (or OCCS_PASSWORD_ENC + OCCS_PASSWORD_KEY).'));
    exitWithErrorBell();
  }
  ...
  for (let i = 0; i < conversionJobs.length; i += 1) {
    ...
    try {
      ...
      fs.writeFileSync(job.outputPath, JSON.stringify(convertedJson, null, 2));
