...
async function apiPutAssemblyTemplate(session, versionUuid, assemblyTemplate, transactionConfigId, opts = {}) {
  ...
  const jsonText = `${JSON.stringify(assemblyTemplate, null, 2)}\n`;
...
function buildManifest({ session, pkg, version, versionMaster, assemblyTemplate, bundlePath }) {
...
export async function packageSaveCommand(bundleDir, cmd) {
  ...
  return runCommand(opts, async () => {
    ...
    if (!dryRun && changes.versionMaster) {
    ...
    if (!dryRun && changes.assemblyTemplate) {
