...
export async function listChartsCommand(cmd) {
  ...
  for (const item of charts) await downloadChart(session, item, outputDir, cmd.verbose);

  console.log(`✅ Saved ${charts.length} charts to ${outputDir}`);
}

async function downloadChart(session, item, outputDir, verbose, onlyActiveVersions = false) {
    ...
    const master = await saveArtifact(session, `${CHART_API}/ChartMasterConfig/${uuid}`, chartDir, `${shortName}_master.json`, verbose);
    ...
    for (const entry of (onlyActiveVersions ? activeVersions(versions, 'ChartVersionConfigRec') : versions)) {
      ...
      const version = await saveArtifact(session, `${CHART_API}/ChartVersionMasterConfig/${versionUuid}`, versionDir, `${versionName}.json`, verbose);
      if (version) await saveSeriesArtifacts(session, version, versionDir, verbose);
    }
  return true;
}
...
export async function getChartCommand(shortName, cmd) {
  ...
  await downloadChart(session, chart, outputDir, cmd.verbose, true);
