...
async function saveArtifact(session, endpoint, directory, fileName, verbose, report) {
  // CCS can retain chart relations after the referenced chart artifact has
  // been deleted. Those 404s are harmless to the cache and must not make a
  // complete export fail (or block downstream mockup generation).
  const artifact = await getArtifact(session, endpoint, { depth: true }, verbose, report, { ignoreNotFound: true });
...
async function saveSeriesArtifacts(session, version, versionDir, verbose, report) {
  ...
  for (const relation of plotSeries) {
    ...
    if (series === null) { complete = false; continue; }
    ...
    if (categoryUuid) {
      ...
      if (category === null) { complete = false; continue; }
      ...
      if (categoryAxisUuid) {
        ...
        ensureDir(axisDir);
        if (await saveArtifact(session, `${CHART_API}/ChartCategoryAxisMasterConfig/${categoryAxisUuid}`, axisDir, `${categoryAxisUuid}.json`, verbose, report) === null) complete = false;
      }
    }
    ...
    if (seriesAxisUuid) {
      ...
      ensureDir(axisDir);
      if (await saveArtifact(session, `${CHART_API}/ChartSeriesAxisMasterConfig/${seriesAxisUuid}`, axisDir, `${seriesAxisUuid}.json`, verbose, report) === null) complete = false;
    }
    ...
    for (const annotation of annotations) {
      ...
      if (await saveArtifact(session, `${CHART_API}/ChartAnnotationMasterConfig/${annotationUuid}`, annotationDir, `${annotationUuid}.json`, verbose, report) === null) complete = false;
    }
  }
  return complete;
}
...
export async function listChartsCommand(cmd) {
  ...
  const results = await mapWithConcurrency(charts, cmd.concurrency ?? 4, async (item) => {
    ...
    // A malformed summary row cannot be downloaded. Treat it like the stale
    // chart relations above: CCS owns the bad record, and it must not block
    // a cache refresh or its downstream mockups.
    if (!uuid) {
      console.warn('⚠ Ignored malformed chart index row (missing UUID).');
...
async function downloadChart(session, item, outputDir, verbose, onlyActiveVersions = false, report) {
    ...
    if (master === undefined) return true;
    ...
    for (const entry of (onlyActiveVersions ? activeVersions(versions, 'ChartVersionConfigRec') : versions)) {
      ...
      if (version === null) complete = false;
      else if (version) complete = (await saveSeriesArtifacts(session, version, versionDir, verbose, report)) && complete;
    }
  return complete;
}
