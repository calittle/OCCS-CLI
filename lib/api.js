...
/**
 * Upload one binary/multipart resource while retaining token refresh. The POST
 * is intentionally not retried after a transient response: an OCCS server may
 * have accepted the blob even when the client did not receive that response.
 */
export async function postMultipart(session, url, body, {
  headers = {},
  verbose = false,
  timeout,
} = {}) {
  const fullUrl = `${session.baseUrl}${url}`;
  if (verbose) console.log(chalk.gray(`→ POST ${fullUrl}`));
  const request = () => axios.post(fullUrl, body, {
    headers: {
      Authorization: `Bearer ${session.token}`,
      Accept: 'application/json',
      ...headers,
