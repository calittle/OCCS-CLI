...
function refreshSessionTokenOnce(session, options) {
  const existing = refreshesInFlight.get(session);
  if (existing) return existing;
  const pending = refreshSessionToken(session, options).finally(() => refreshesInFlight.delete(session));
  refreshesInFlight.set(session, pending);
  return pending;
}
...
export async function retryUnauthorizedRequest(session, request, {
  ...
  for (let attempt = 1; attempt <= SESSION_REFRESH_ATTEMPTS; attempt += 1) {
    ...
    try {
      // Concurrent exports can all observe one expired token. Share the
      // refresh so they do not race to overwrite the saved session.
      await (refresh === refreshSessionToken ? refreshSessionTokenOnce(session, { timeout }) : refresh(session, { timeout }));
    } catch (refreshError) {
...
/**
 * Send a JSON mutation through the token-refresh path used by reads.
 * Only PUT receives transient retries: retrying a POST after a timeout or 5xx
 * could create a second OCCS resource when the first request actually landed.
 * Resource-specific commands own their payload and error reporting; this helper
 * deliberately has no OCCS domain knowledge.
 */
export async function mutateJson(session, method, url, payload, {
  headers = {},
  verbose = false,
  timeout,
} = {}) {
  ...
  if (!['post', 'put'].includes(normalizedMethod)) {
    throw new Error(`Unsupported JSON mutation method: ${method}`);
  }

  const fullUrl = `${session.baseUrl}${url}`;
  if (verbose) console.log(chalk.gray(`→ ${normalizedMethod.toUpperCase()} ${fullUrl}`));
  ...
  const send = normalizedMethod === 'put'
    ? () => retryTransientRequest(request, { url })
    : request;
  const res = await retryUnauthorizedRequest(session, send, { url, timeout });
  return res.data;
}

/** Delete an OCCS JSON resource with the same authorization-refresh behavior as writes. */
export async function deleteJson(session, url, {
  headers = {},
  verbose = false,
  timeout,
} = {}) {
  const fullUrl = `${session.baseUrl}${url}`;
  if (verbose) console.log(chalk.gray(`→ DELETE ${fullUrl}`));
  ...
  const res = await retryUnauthorizedRequest(session, request, { url, timeout });
  return res.data;
}

/**
 * Upload one binary/multipart resource while retaining token refresh. POST is
 * not retried after a transient response because OCCS may have accepted the
 * blob even when the client did not receive that response; PUT is retried.
 */
async function sendMultipart(session, method, url, body, {
  headers = {},
  verbose = false,
  timeout,
} = {}) {
  const fullUrl = `${session.baseUrl}${url}`;
  if (verbose) console.log(chalk.gray(`→ ${method.toUpperCase()} ${fullUrl}`));
  const request = () => axios({
    method,
    url: fullUrl,
    data: body,
...
export async function postMultipart(session, url, body, options = {}) {
  return sendMultipart(session, 'post', url, body, options);
}

/** Replace an existing binary/multipart resource. */
export async function putMultipart(session, url, body, options = {}) {
  return sendMultipart(session, 'put', url, body, options);
}
